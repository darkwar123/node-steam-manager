 ...
 * @param {Object} manager - steam-trade-offer-manager
 * @param {String} [steamID] - user SteamID64
 * @param {Number} [appID] - game appID
 * @param {Number} [contextID] - game contextID
 * @return {Promise}
 */
function loadUserInventory(manager, { steamID, appID, contextID }) {
	return new Promise((resolve, reject) => {
		manager.getUserInventoryContents(steamID, appID, contextID, true, (err, items) => {
			if (err) {
				return reject(err);
			}
...
SteamManager.prototype.loadInventory = function loadInventory({ steamID, appID, contextID } = { }) {
	return new Promise((resolve, reject) => {
		...
		let stack = appID['map']((value) => {
			return loadUserInventory(this._manager, {
				steamID,
				contextID,
				appID: value,
			});
