...
SteamManager.prototype.getReceivedItems = function (offer, count) {
    ...
    offer.getReceivedItems((err, items) => {
        if (err) {
            this._handleError(err);
            return setTimeout(() => this.getReceivedItems(offer, count - 1), GETRECEIVEDITEMS_REPEAT_TIME);
        }
...
SteamManager.prototype.cancelOffer = function cancelOffer (offer, count) {
    ...
    offer.cancel(err => {
        if(err){
            this._handleError(err);
            return setTimeout(() => this.cancelOffer(offer, count - 1), CANCEL_REPEAT_TIME);
        }
...
SteamManager.prototype.acceptOffer = function acceptOffer (offer, count) {
    ...
    offer.itemsToReceive.forEach(item => {
        if (this.appID.indexOf(Number(item.appid)) === -1) {
    ...
    offer.accept(true, err => {
        if(err){
            this._handleError(err);
            return setTimeout(() => this.acceptOffer(offer, count - 1), ACCEPT_REPEAT_TIME);
        }
