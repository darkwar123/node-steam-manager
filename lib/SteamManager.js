 ...
 * @param {Number|Number[]} [appID] - game appID you want to handle or array of appIDs you wan't to use.
 ...
 * @param {Number} [pollInterval] - the time, in milliseconds, between polls. If -1, timed polling is disabled. Minimum 1000, default 15000 (15 seconds).
...
let SteamManager = function SteamManager ({  accountName, password, sharedSecret, identitySecret, apiKey, steamID,
    ...
    this.appID = Array.isArray(appID) ? appID : [ appID ];
...
SteamManager.prototype.setCookies = function setCookies (cookies) {
    ...
    const cookiesFile = 'cookies_' + this.steamID.getSteamID64() + '.json';
